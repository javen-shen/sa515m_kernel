...
static int quectel_sensor_poweron(struct i2c_client *client)
{
    ...
    if (gpio > 0) {
        printk("iam20680 sensor power gpio =%d \n", gpio);
        err = gpio_request(gpio, "qcom,power-gpio");
        if (err) {
            printk("iam20680 gpio_requet failed\n");
        }

        err = gpio_direction_output(gpio, 1);
        if (err) {
            printk("iam20680 gpio_direction failed\n");
            gpio_free(gpio);
        }
        msleep(100);

    } else {
        printk(" iam20680 failed to get of node qcom,power-gpio gpio=%d \n", gpio);
    }
