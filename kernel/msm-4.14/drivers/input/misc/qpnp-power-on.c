...
static int qpnp_pon_sw_reset(struct qpnp_pon *pon)
{
	int rc;

	rc = qpnp_pon_masked_write(pon, 0x862, 0xF, 1);
	usleep_range(100, 120);	

	rc |= qpnp_pon_masked_write(pon, 0x863, 0x80, 0x80);
	usleep_range(100, 120);

	rc |= qpnp_pon_masked_write(pon, 0x864, 0xFF, 0xA5);
	msleep(10000);
...
int ql_system_reset(void)
{
	...
	rc = qpnp_pon_sw_reset(sys_reset_dev);

	if (rc) {
		dev_err(sys_reset_dev->dev, "qpnp_pon_sw_reset, rc=%d\n",
                        rc);
                return rc;
	}
