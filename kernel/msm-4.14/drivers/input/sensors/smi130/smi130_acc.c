...
//add by bowen 2019/12/11:add gpio control power on sensor
#include <linux/of.h>
...
/* aded by bowen.gu 2019/09/03 AG550 use gpio pin to control sensor's power*/
static int quectel_sensor_poweron(struct i2c_client *client)
{
    ...
    if (gpio > 0) {
        printk("smi130 sensor power gpio =%d \n", gpio);
        err = gpio_request(gpio, "qcom,power-gpio");
        if (err) {
            printk(" smi130 gpio_requet failed\n");
        }

        err = gpio_direction_output(gpio, 1);
        if (err) {
            printk(" smi130 gpio_direction failed\n");
            gpio_free(gpio);
        }    
        msleep(100);

    } else {
        printk(" smi130 failed to get of node qcom,power-gpio gpio=%d \n", gpio);
    }    
