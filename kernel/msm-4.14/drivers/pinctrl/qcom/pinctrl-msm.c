...
//larryzhang-20200225:check support wakeup pin function
static struct msm_pinctrl *ql_pctrl = NULL;
int ql_find_wakeup_in_from_tab(int gpio){
    int i;
    int	j;
    int	len = 0;
    int ret;
    ...
    int ql_support_awake_pin_tab[ql_pctrl->soc->n_gpio_mux_in];
    

    for (i = 0; i < ql_pctrl->soc->n_gpio_mux_in; i++) {
            struct msm_gpio_mux_input *gpio_in = &ql_pctrl->soc->gpio_mux_in[i];
                if (gpio_in->init == 1) {
			ql_support_awake_pin_tab[len++] = gpio_in->gpio;
			if(gpio == gpio_in->gpio){
				return 0;
			}				
		}
     }

     pr_warn("%s:This gpio%d not support awake function, support tab: ", __func__,gpio);
     for( j = 0 ;j < len ; j++){
           pr_warn("gpio%d ,",ql_support_awake_pin_tab[j]);
     }
     pr_warn("\n");

     return len;
}
EXPORT_SYMBOL(ql_find_wakeup_in_from_tab);
//end
...
//larryzhang-20200225:add wakeup function of gpio
	ql_pctrl = pctrl;
//end
	return 0;
}
