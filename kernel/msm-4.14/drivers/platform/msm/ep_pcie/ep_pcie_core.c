...
static int ep_pcie_probe(struct platform_device *pdev)
{
	...
	if (ql_get_net_interface() == QL_DATA_INTERFACE_USB) {
		return EPERM;
	}
#endif
...
/*========================================
    Quectel Features.
    1. EXPORT PCIE function for other module.
    2. This link status was set by PBL.
========================================*/
void ep_pcie_deinit(void)
{
	pr_debug("%s\n", __func__);
...
EXPORT_SYMBOL(ep_pcie_deinit);

u32 ep_pcie_linkstatus(void)
{
	u32 val = 0;
	struct ep_pcie_dev_t *dev = &ep_pcie_dev;
...
EXPORT_SYMBOL(ep_pcie_linkstatus);
#endif
