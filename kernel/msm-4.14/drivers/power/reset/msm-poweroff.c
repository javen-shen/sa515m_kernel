...
extern void ep_pcie_deinit(void);
extern unsigned int ep_pcie_linkstatus(void);
#endif

#ifdef QL_DATA_INTERFACE
extern int ql_system_reset(void);
#endif
...
/*========================================
    Quectel Features.
    1. EDL MODE 0x01 is for USB.
    2. EDL MODE 0X04 is for PCIE.
========================================*/
#define SCM_EDLOAD_MODE_USB			0X01
#define SCM_EDLOAD_MODE_PCIE		0X04
#else
...
static void enable_emergency_dload_mode(void)
{
	...
	/*========================================
		...
		1. EDL MODE 0x01 is for USB.
		2. EDL MODE 0X04 is for PCIE.
		3. PCIE link status is 1 enter PCIE EDL.
	========================================*/
	if(ep_pcie_linkstatus() && ql_get_diag_interface() == QL_DATA_INTERFACE_PCIE)
	{
		ret = scm_set_dload_mode(SCM_EDLOAD_MODE_PCIE, 0);
		if (ret)
	...
	{
		ret = scm_set_dload_mode(SCM_EDLOAD_MODE_USB, 0);
		if (ret)
			pr_err("Failed to set secure EDLOAD mode: %d\n", ret);
	}
#else
...
static void do_msm_restart(enum reboot_mode reboot_mode, const char *cmd)
{
	...
	if (cmd) {
		pr_notice("do_msm_restart cmd is %s\n", cmd);
	}
