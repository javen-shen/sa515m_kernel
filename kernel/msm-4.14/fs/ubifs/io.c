...
{
	/*
	*add by dawn 2020-04-02
	*ubifs is forbidden to be mounted in read-only mode 
	*because this will cause the application to run abnormally
	*/
	ubifs_err(c, "quectel prevents ubifs mount read-only mode, error %d", err);
#if 0
	if (!c->ro_error) {
		c->ro_error = 1;
		c->no_chk_data_crc = 0;
