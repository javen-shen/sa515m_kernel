...
static int msm_cdc_pinctrl_probe(struct platform_device *pdev)
{
	int ret = 0;
	struct msm_cdc_pinctrl_info *gpio_data;
	printk("enter into msm_cdc_pinctrl_probe ...\n");
...
static int __init msm_cdc_pinctrl_drv_init(void)
{
       platform_driver_register(&msm_cdc_pinctrl_driver);
        return 0;
}
static void __exit msm_cdc_pinctrl_drv_exit(void)
{
       platform_driver_unregister(&msm_cdc_pinctrl_driver);
}

module_init(msm_cdc_pinctrl_drv_init);
module_exit(msm_cdc_pinctrl_drv_exit);
MODULE_DESCRIPTION(" pinctrl for sdx55");
